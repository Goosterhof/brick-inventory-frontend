...
describe("dialog service", () => {
    ...
    describe("open", () => {
        ...
        it("should set UnoCSS attributes on the dialog element", async () => {
            // Arrange
            const service = createDialogService();
            const wrapper = shallowMount(service.DialogContainerComponent);
            ...
            expect(dialog.attributes("p")).toBe("0");
            expect(dialog.attributes("m")).toBe("auto");
            expect(dialog.attributes("bg")).toBe("transparent");
            expect(dialog.classes()).toContain("backdrop:bg-black");
        });
